...
import { DEMO_OWNER, DEMO_ALT_EMPLOYEE, uniqueId } from '../helpers/ids';
...
test.describe('Tenant Isolation', () => {
  test('resource created in tenant A is not accessible from tenant B', async () => {
    ...
    const ownerB = await loginApi(apiURL, DEMO_ALT_EMPLOYEE.email, DEMO_ALT_EMPLOYEE.password);

    const runId = uniqueId();
    const createRes = await ownerA.api.post('tenants/toko-maju/customers', {
      data: { code: `ISO-${runId.slice(-12)}`, name: `Isolation-${runId}`, email: '', phone: '', address: '' },
    });
  ...
  test('list from tenant B does not contain tenant A resources', async () => {
    ...
    const runId = uniqueId();
    const createRes = await ownerA.api.post('tenants/toko-maju/customers', {
      data: { code: `ISL-${runId.slice(-12)}`, name: `Isolation-List-${runId}`, email: '', phone: '', address: '' },
    });
    expect(createRes.ok()).toBeTruthy();
