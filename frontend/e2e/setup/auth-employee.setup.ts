...
setup('setup tenant employee storage state', async ({ page }) => {
  ...
  const loginRes = await ctx.post('auth/login', { data: { email: DEMO_EMPLOYEE.email, password: DEMO_EMPLOYEE.password } });
  const loginBody = await loginRes.json();
  console.log('Employee login OK:', loginBody.user.email);
  await ctx.dispose();
