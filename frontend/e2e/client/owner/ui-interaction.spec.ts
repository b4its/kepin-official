...
test.describe('Owner UI Interaction', () => {
  ...
  test('dashboard period comparison fills compare range', async ({ page }) => {
    const errors: string[] = [];
    page.on('response', (res) => { if (res.status() >= 500) errors.push(`${res.status()} ${res.url()}`); });

    await page.goto(`/app/${TENANT}`);
    await page.waitForLoadState('networkidle');
    await expect(page.getByText('Kas & Bank')).toBeVisible();
  ...
  test('reconciliation page shows bank accounts and transactions tables', async ({ page }) => {
    ...
    await expect(page.getByLabel('Rekening:')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Cocokkan Semua Saran' })).toBeVisible();
  });
});
