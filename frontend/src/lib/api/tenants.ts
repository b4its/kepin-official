...
export async function createPosCheckout(slug: string, data: { items: { product_id: string; quantity: string }[]; reason?: string; amount_paid?: string }) { return api(`/tenants/${slug}/pos/checkout`, { method: 'POST', body: JSON.stringify(data) }); }
export async function getPosTransactions(slug: string, search?: string, pageSize?: number, page?: number) { return api(`/tenants/${slug}/pos/transactions?${search ? `search=${encodeURIComponent(search)}&` : ''}${pageSize ? `pageSize=${pageSize}&` : ''}${page ? `page=${page}` : ''}`); }
